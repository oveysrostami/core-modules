...
class WalletPurchaseRejectedEvent implements ShouldBeLogged, LoggableEventPayloadInterface
{
    use Dispatchable, InteractsWithSockets, SerializesModels, ShouldBeLoggedTrait;

    public function __construct(public WalletPurchaseRequest $request)
    {
        $this->initEventUUID();
    }
    ...
    {
        $request = WalletPurchaseRequest::findOrFail($payload['request_id']);
